...
import { signUp, clearSuccess, clearError } from "@/actions/auth";
...
function SignUpForm({
    ...
    const handleSubmitUserInfo = () => {
        if (userInfo.password.length < 8) {
            return toast.error("Password is not in right formation", {
                duration: 2000
            })
        }
    ...
    return (
        <div className="md:w-1/2 md:flex items-center justify-center px-6 py-24 relative">       {/* Left side - Sign-up form */}
            {/* Logo */}
            ...
            <div className="w-full max-w-sm space-y-6 m-auto">
                ...
                <div className="space-y-4 mb-6">
                    {/* Email input */}
                    <Input disable={isLoading} id="email" placeholder="Email" type="email" name="email" value={userInfo.email} onChange={handleUserInfo} />

                    {/* Password input */}
                    <div className="space-y-2">
                        <Input disable={isLoading} id="password" placeholder="Password" type="password" name="password" value={userInfo.password} onChange={handleUserInfo} />
                ...
                <div className="flex flex-col space-y-4">
                    <Button className="w-full bg-black text-white hover:cursor-pointer"
                        onClick={() => handleSubmitUserInfo()}
                        disable={isLoading}
                    >
                        Sign up {isLoading && <LoaderCircle strokeWidth={4} className="animate-spin" />}
                    </Button>
                    <p className="text-sm text-center text-muted-foreground">
                        Already have account?{" "}
                        <Link className="underline text-foreground cursor-pointer"
                            to='/login'
                            disable={isLoading}
                        >
                            Sign in
