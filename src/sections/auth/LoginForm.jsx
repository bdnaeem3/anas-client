...
import { login, clearSuccess, clearError } from "@/actions/auth";
import { toast } from "sonner";
import { LoaderCircle } from "lucide-react";

function LoginForm({
    loginWithGoogle,
    Auth: { message, error, isLoading },
    login,
    clearSuccess,
    clearError,
}) {
            ...
            <div className="w-full max-w-sm space-y-6 m-auto">
                ...
                <div className="space-y-2 text-center">
                    ...
                    <p className="text-muted-foreground text-sm">
                        Let's get started. Fill in the details below to login to your
                        account.
                    </p>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    {/* <Button variant="outline" className="w-full hover:cursor-pointer" onClick={() => loginWithGoogle()}> */}
                ...
                <div className="space-y-4">
                    <Input disable={isLoading} value={userInfo.email} onChange={handleUserInfo} name="email" type="email" placeholder="Email" />
                    <Input disable={isLoading} value={userInfo.password} onChange={handleUserInfo} name="password" type="password" placeholder="Password" />
                </div>
                ...
                <div className="flex justify-between">
                    ...
                    <Link disable={isLoading}
                        to='/forget-password' className="text-sm text-muted-foreground hover:text-foreground underline">
                        Forgot password?
                    </Link>
                </div>
                ...
                <div className="flex flex-col space-y-4">
                    ...
                    >
                        Sign In {isLoading && <LoaderCircle strokeWidth={4} className="animate-spin" />}
                    </Button>
                    <p className="text-sm text-center text-muted-foreground">
                        Already have account?{" "}
                        <Link className="underline text-foreground cursor-pointer" to='/signup'>
                            Sign up
                        </Link>
                    </p>
                </div>
